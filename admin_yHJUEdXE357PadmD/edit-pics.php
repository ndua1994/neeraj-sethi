...
if(isset($_POST['add_pics']))
{
	...
	if(!empty($_FILES['pic_img']['name']))
	{
	$pic_img='img_'.rand(0,10000).'_'.$_FILES['pic_img']['name'];
	}
	else
	{
	$pic_img=$_POST['pic_img_hidden'];
	}



	$query=mysqli_query($conn,"update tbl_pics 
		                                               SET cat_id='$cat_id',
		                                                   pic_img='$pic_img',
		                                                   pic_img_alt='$pic_img_alt',
		                                                   is_active='$is_active' where pic_id='".base64_decode($_GET['edit'])."'");
	if($query)
	{
		if(!empty($_FILES['pic_img']['name']))
	{
	unlink('uploads/'.$_POST['pic_img_hidden'].'');
...
$details=mysqli_fetch_array(mysqli_query($conn,"select * from tbl_pics where pic_id=".base64_decode($_GET['edit']).""));
...
<h3 class="card-label">Edit Pics</h3>
</div>
...
<select name="cat_id" class="form-control">
	    ...
	    {
	    ?>
	    <option value='<?=$row['cat_id']?>' <?=($row['cat_id']==$details['cat_id'] ? 'selected' : '')?>><?=$row['cat_name']?></option>
	    <?php 
	    }
	    ?>
</select>
...
<div class="mt-2">
<img src='<?=BASE_URL?>uploads/<?=$details['pic_img']?>' width='80'>	
</div>
</div>
<div class="form-group">
<label for="exampleInputPassword1">Image Alt Tag
</label>
<input type="text" name="pic_img_alt" value="<?=$details['pic_img_alt']?>" class="form-control" placeholder="Image Alt Tag">
</div>
...
<div class="card-footer">
	<input type="hidden" name="pic_img_hidden" value="<?=$details['pic_img']?>">
<button type="submit" name="add_pics" class="btn btn-primary mr-2">Update</button>
