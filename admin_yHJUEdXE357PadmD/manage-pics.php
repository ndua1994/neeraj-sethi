...
if(isset($_GET['del']))
{
	...
	$img_details=mysqli_fetch_array(mysqli_query($conn,"select * from tbl_pics where pic_id=$del_id"));
	unlink('uploads/'.$img_details['pic_img'].'');
	$query=mysqli_query($conn,"delete from tbl_pics where pic_id=$del_id");
	if($query)
	{
		header('Location:manage-pics.php');
	}
}
?>
	<!--begin::Body-->
...
<h3 class="card-label">Manage Pics</h3>
</div>
<div class="card-toolbar">
<!--begin::Button-->
<a href="<?=BASE_URL?>add-pics.php" class="btn btn-primary font-weight-bolder">
...
<table id="example" class="table table-bordered dataTable " style="width:100%">
		...
		<?php 
		$key=0;
		$query=mysqli_query($conn,"select tc.cat_name,tp.* from tbl_pics as tp
		...
		<td><?=$row['cat_name']?></td>
		<td>
		<img src='<?=BASE_URL.'uploads/'.$row['pic_img'].''?>' width='120'></td>
		...
		<td><a href="<?=BASE_URL?>edit-pics.php?edit=<?=base64_encode($row['pic_id'])?>" class="btn btn-warning">Edit</a></td>
		<td><a href="javascript:void(0)" data-url="<?=BASE_URL?>manage-pics.php?del=<?=base64_encode($row['pic_id'])?>" class="btn btn-danger del_rec">Delete</a></td>
		</tr>
		<?php } ?>  
        </tbody>
        <tfoot>
